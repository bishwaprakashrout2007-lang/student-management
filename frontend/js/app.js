...
async function handleLogin(e) {
  ...
  try {
    ...
    showToast('Network error. Please check backend connection.', true);
  }
}
      ...
      if (stats.recent_students.length === 0) {
        ...
        stats.recent_students.forEach(student => {
          ...
          const avatarHTML = student.profile_image
            ? `<img src="${API_BASE_URL}${student.profile_image}" class="recent-avatar" alt="${student.full_name}">`
  ...
  classChartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: data,
        backgroundColor: [
          '#6366f1', '#10b981', '#0ea5e9', '#f59e0b', '#ec4899', '#8b5cf6', '#3b82f6'
        ],
        borderWidth: 2,
        borderColor: '#1e293b'
      }]
...
async function searchStudents() {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/students?search_by=${searchBy}&search_term=${encodeURIComponent(searchTerm)}`);
    const data = await response.json();
    if (data.success) {
      populateStudentsTable(data.students);
    }
...
function populateStudentsTable(students) {
  ...
  students.forEach(student => {
    ...
    const photoHTML = student.profile_image
      ? `<img src="${API_BASE_URL}${student.profile_image}" class="student-photo" alt="Profile">`
...
function openModal(mode, studentData = null) {
  ...
  if (mode === 'add') {
    ...
    if (studentData.profile_image) {
      ...
      preview.innerHTML = `<img src="${API_BASE_URL}${studentData.profile_image}" alt="Preview"><span>Replace profile image</span>`;
    }
  }
...
async function handleStudentSubmit(e) {
  ...
  const url    = isEdit ? `${API_BASE_URL}/api/students/${studentId}` : `${API_BASE_URL}/api/students`;
